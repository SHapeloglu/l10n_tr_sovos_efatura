...
# GİB kodları — aynı UUID ile tekrar gönder
RETRY_SAME_UUID = {
...
class ResendInvoiceWizard(models.TransientModel):
    ...
    def _onchange_invoice(self):
        if self.invoice_id:
            ...
            if code in RETRY_SAME_UUID:
                self.resend_type = 'same_uuid'
            else:
                self.resend_type = 'new_invoice'
    ...
    def action_resend(self):
        ...
        if invoice.x_efatura_status not in ('error', 'rejected'):
            raise UserError(_('Sadece hatalı veya reddedilmiş faturalar tekrar gönderilebilir.'))

        if self.resend_type == 'same_uuid':
            return self._resend_same_uuid(invoice)
        else:
            raise UserError(_('İçerik değişikliği için önce faturayı iptal edin, ardından yeni fatura kesin.'))

    def _resend_same_uuid(self, invoice):
        """Aynı UUID ile UBL yeniden oluşturur ve Sovos'a gönderir."""
        ...
        try:
            ...
            _logger.info('Tekrar gönderim başarılı: %s (aynı UUID)', uuid)
        except Exception as e:
